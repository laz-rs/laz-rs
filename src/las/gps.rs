...
pub mod v1 {
    ...
    impl<R: Read> FieldDecompressor<R> for GpsTimeDecompressor {
        ...
        fn decompress_with(
            ...
            if let Some(last) = self.last_gps_time.as_mut() {
                let multi;
                if self.last_gps_time_diff == 0 {
                    ...
                    if multi == 1 {
                        ...
                        last.value += self.last_gps_time_diff as i64;
                    } else if multi == 2 {
                        // the difference is huge,
                        // the gps was written as is
                        last.value = decoder.read_int_64()? as i64;
                    }
                } else {
                    ...
                    if multi < LASZIP_GPS_TIME_MULTI_MAX - 2 {
                        ...
                        last.value += gps_time_diff as i64;
                    } else if multi < LASZIP_GPS_TIME_MULTI_MAX - 1 {
                        last.value = decoder.read_int_64()? as i64;
                    }
                }
                GpsTime::pack_into(last, &mut buf);
            } else {
                decoder.in_stream().read_exact(&mut buf)?;
                self.last_gps_time = Some(GpsTime::unpack_from(&mut buf));
            }
            Ok(())
        }
    }
    ...
    impl<W: Write> FieldCompressor<W> for GpsTimeCompressor {
        ...
        //TODO see if it is possible to have constants for the different magic numbers of the multi
        //  symbol, for clarity of the code
        fn compress_with(
            &mut self,
            mut encoder: &mut ArithmeticEncoder<W>,
            buf: &[u8],
        ) -> std::io::Result<()> {
            let current_gps_time = GpsTime::unpack_from(buf);
            if let Some(last) = self.last_gps_time.as_mut() {
                if self.last_gps_time_diff == 0 {
                    if current_gps_time.value == last.value {
                        encoder.encode_symbol(&mut self.gps_time_0_diff_model, 0)?;
                    } else {
                        let current_gps_time_diff_64 = current_gps_time.value - last.value;
                        ...
                        if current_gps_time_diff_64 == current_gps_time_diff_32 as i64 {
                            ...
                            encoder.write_int64(current_gps_time.value as u64)?;
                        }
                        last.value = current_gps_time.value;
                    }
                } else {
            ...
            self.last_gps_time = Some(current_gps_time);
            Ok(())
        }
    }
}
...
pub mod v2 {
    ...
    impl<R: Read> FieldDecompressor<R> for GpsTimeDecompressor {
        ...
        fn decompress_with(
                        ...
                        // < LASZIP_GPS_TIME_MULTI
                            ...
                            if multi > LASZIP_GPS_TIME_MULTI_MINUS {
                                ...
                                if self.common.multi_extreme_counters[self.common.last as usize] > 3
                                {
                                    self.common.last_gps_time_diffs[self.common.last as usize] =
                                        gps_time_diff;
                                    self.common.multi_extreme_counters[self.common.last as usize] =
                                        0;
                                }
                            }
                        }
            ...
            self.common.last_gps_times[self.common.last as usize].pack_into(buf);
            Ok(())
        }
    }
}
